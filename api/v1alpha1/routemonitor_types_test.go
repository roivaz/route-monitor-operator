...
func TestRouteMonitor_GetResolvedDomain(t *testing.T) {
	tests := []struct {
		name               string
		...
		{
			name:               "RHOBS + unset DomainRef -> HCP",
...
func TestRouteMonitor_GetResolvedServiceMonitorType(t *testing.T) {
	tests := []struct {
		name                       string
		serviceMonitorType         string
		domainRef                  ClusterDomainRef
		...
		{
			name:                       "unset ServiceMonitorType + HCP DomainRef -> CoreOS",
			serviceMonitorType:         "",
			domainRef:                  ClusterDomainRefHCP,
			expectedServiceMonitorType: ServiceMonitorTypeCoreOS,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result := rm.GetResolvedServiceMonitorType()
